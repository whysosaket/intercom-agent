...
export function EvalReport({ stats }: EvalReportProps) {
  ...
  return (
    <div className="flex items-center gap-7 flex-wrap px-5 py-3 border-b border-cream-200 bg-elevated">
      <StatItem value={stats.total} label="Generated" className="text-cream-700" />
      ...
      <StatItem value={avgConfDisplay} label="Avg confidence" className="text-cream-700" />
      {stats.pending > 0 && <StatItem value={stats.pending} label="Pending" className="text-accent-500" />}
    </div>
  )
}
